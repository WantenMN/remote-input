...
async fn main() -> anyhow::Result<()> {
    ...
    let https_url = format!("https://{}:{}", local_ip, port + 1);
    ...
    eprintln!("  ║ → {:<width$} ║", http_url, width = w - 4);
    eprintln!("  ║ → {:<width$} ║", https_url, width = w - 4);
    eprintln!("  ║ {:<width$} ║", "", width = w - 2);
    eprintln!("  ║ {:<width$} ║", "In public places (cafe, library, etc.),", width = w - 2);
    eprintln!("  ║ {:<width$} ║", "use HTTPS to protect your input from", width = w - 2);
    eprintln!("  ║ {:<width$} ║", "being intercepted by others on the LAN.", width = w - 2);
    eprintln!("  ║ {:<width$} ║", "", width = w - 2);
    eprintln!("  ║ {:<width$} ║", "The browser may warn that the HTTPS", width = w - 2);
    eprintln!("  ║ {:<width$} ║", "certificate is untrusted. This is normal", width = w - 2);
    eprintln!("  ║ {:<width$} ║", "because it is a temporary self-signed cert", width = w - 2);
    eprintln!("  ║ {:<width$} ║", "generated by this program. It is not a", width = w - 2);
    eprintln!("  ║ {:<width$} ║", "security threat. On the contrary, it", width = w - 2);
    eprintln!("  ║ {:<width$} ║", "ensures your communication is encrypted", width = w - 2);
    eprintln!("  ║ {:<width$} ║", "and cannot be viewed by others.", width = w - 2);
    eprintln!("  ║ {:<width$} ║", "", width = w - 2);
    ...
    let app_http = Router::new()
        ...
        .with_state(AppState { paste_tx: paste_tx.clone() });

    let http_addr = SocketAddr::from(([0, 0, 0, 0], port));
    let http_listener = tokio::net::TcpListener::bind(http_addr).await?;
    tokio::spawn(async move {
        if let Err(e) = axum::serve(http_listener, app_http)
    ...
    let https_addr = SocketAddr::from(([0, 0, 0, 0], port + 1));
